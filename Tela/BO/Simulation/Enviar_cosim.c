...
void AfterSimInit(void){
	...
	#endif
	A661ConnectServer("127.0.0.1", 1231, "C:/Users/Goku/Desktop/scade/BO/BO/BO/Simulation/Enviar_UA_1.bin", "C:/Users/Goku/Desktop/scade/BO/BO/BO/Simulation/Enviar_UA_1_conf.xml" );
}
...
void BeforeSimStep(void){
	...
	/* reveive from server */
	Enviar_UA_1_receive_clear(&inputs_ctx, NULL);
	if (A661Receive(msg, sizeof msg) == 0)
		Enviar_UA_1_receive(msg, sizeof msg, &inputs_ctx, NULL);
}
...
void AfterSimStep(void){
	...
	int len;
	len = Enviar_UA_1_send(msg, &outputs_ctx, NULL);
	/* Send to server */
	A661Send(msg, len);
}
