...
A661_INTERNAL int a661_com_A661_CMD_UA_REQUEST_layer_1(buffer_el msg[], 
  outC_UTI* scade_ctx,  extern_out_ctx_t * extern_ctx){
  int n1 = 0;
...
A661_INTERNAL int a661_com_layer_1(buffer_el msg[],  outC_UTI* scade_ctx, 
  ...
  scade_ctx->LayerVisible){
    ...
    if (scade_ctx->EmitToCDS){
      WIDGET("Ativos_EditBoxNumeric#2");
      n1 = a661_com_csp_widget_EditBoxNumeric(msg + n0, 2, scade_ctx->EmitToCDS,
        scade_ctx->ActiveBedsCount);
    }
    if (scade_ctx->EmitToCDS){
      WIDGET("Livre_EditBoxNumeric#3");
      n2 = a661_com_csp_widget_EditBoxNumeric(msg + n0 + n1, 3,
        scade_ctx->EmitToCDS, scade_ctx->FreeBedsCount);
    }
    if (scade_ctx->EmitToCDS){
      WIDGET("Leito0_GpRectangle#6");
      n3 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2, 6,
        scade_ctx->EmitToCDS, scade_ctx->BedColor1);
    }
    if (scade_ctx->EmitToCDS){
      WIDGET("Leito1_GpRectangle#9");
      n4 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3, 9,
        scade_ctx->EmitToCDS, scade_ctx->BedColor2);
    }
    if (scade_ctx->EmitToCDS){
      WIDGET("Leito2_GpRectangle#12");
      n5 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4, 12,
        scade_ctx->EmitToCDS, scade_ctx->BedColor3);
    }
    if (scade_ctx->EmitToCDS){
      WIDGET("Leito3_GpRectangle#15");
      n6 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5,
        15, scade_ctx->EmitToCDS, scade_ctx->BedColor4);
    }
    if (scade_ctx->EmitToCDS){
      WIDGET("Leito4_GpRectangle#18");
      n7 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6,
        18, scade_ctx->EmitToCDS, scade_ctx->BedColor5);
    }
    if (scade_ctx->EmitToCDS){
      WIDGET("Leito5_GpRectangle#21");
      n8 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7,
        21, scade_ctx->EmitToCDS, scade_ctx->BedColor6);
    }
    if (scade_ctx->EmitToCDS){
      WIDGET("Leito6_GpRectangle#24");
      n9 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8,
        24, scade_ctx->EmitToCDS, scade_ctx->BedColor7);
    }
    if (scade_ctx->EmitToCDS){
      WIDGET("Leito7_GpRectangle#27");
      n10 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9,
        27, scade_ctx->EmitToCDS, scade_ctx->BedColor8);
    }
    if (scade_ctx->LayerVisible){
      n11 = a661_com_A661_CMD_UA_REQUEST_layer_1(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10,
        scade_ctx, extern_ctx);
    }
    n12 = 4;
    WRITE_ULONG(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11,
      A661_END_BLOCK);
    size = n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12;
    WRITE_ULONG(msg + 4, size);
  }
  return n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 ;
}
 int ua_send(buffer_el msg[],  outC_UTI* scade_ctx, 
  extern_out_ctx_t * extern_ctx){
  int n1 = 0;
...
void ua_receive_clear(inC_UTI* scade_ctx,  extern_in_ctx_t * extern_ctx){
  scade_ctx->ActivateBed1 = FALSE;
  scade_ctx->AllocateBed1 = FALSE;
  scade_ctx->ActivateBed2 = FALSE;
  scade_ctx->AllocateBed2 = FALSE;
  scade_ctx->ActivateBed3 = FALSE;
  scade_ctx->AllocateBed3 = FALSE;
  scade_ctx->ActivateBed4 = FALSE;
  scade_ctx->AllocateBed4 = FALSE;
  scade_ctx->ActivateBed5 = FALSE;
  scade_ctx->AllocateBed5 = FALSE;
  scade_ctx->ActivateBed6 = FALSE;
  scade_ctx->AllocateBed6 = FALSE;
  scade_ctx->ActivateBed7 = FALSE;
  scade_ctx->AllocateBed7 = FALSE;
  scade_ctx->ActivateBed8 = FALSE;
  scade_ctx->AllocateBed8 = FALSE;
}
int ua_receive(buffer_el buffer[],  int buffer_size,  inC_UTI* scade_ctx, 
                  ...
                  case 25 : 
                    ...
                    switch (READ_USHORT(( buffer + ( 2 + n ) ))){
                      case A661_EVT_STATE_CHANGE : 
                        /*  PictureToggleButton_A661_EVT_STATE_CHANGE */
                        /*  Notify */
                        scade_ctx->ActivateBed8 = TRUE;
                        break;
                      default : break;
                    }
                    n = next_n;
