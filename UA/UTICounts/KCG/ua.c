...
/**
** File ..\..\UA\UTICounts\KCG\ua.c
**
** Automatically generated by SCADE Suite UA Adaptor
** Version 6.4.2 (build i25)
**
** Date of generation: 2015-09-24T12:22:26
** Command line:  $ uaadaptor.exe -n 'C:\Program Files (x86)\Esterel Technologies\Esterel SCADE 6.4.2\SCADE Display\config\Workspace\environments\UAPageCreator\A661Norm\a661.xml' -o 'ua' -outdir '..\..\UA\UTICounts\KCG\' -k '..\..\UA\UTICounts\KCG\kcg_trace.xml -d '..\..\DF\UTILeitos.sgfx' ..\..\UA\UTICounts\UTICounts.sdy
**/
...
A661_INTERNAL int a661_com_A661_CMD_UA_REQUEST_layer_1(buffer_el msg[], 
  outC_UTICounts* scade_ctx,  extern_out_ctx_t * extern_ctx){
  int n1 = 0;
...
A661_INTERNAL int a661_com_layer_1(buffer_el msg[],  outC_UTICounts* scade_ctx, 
  ...
  scade_ctx->LayerVisible){
    ...
    if (scade_ctx->EmitToCDS){
      WIDGET("LeitosTotal#2");
      n1 = a661_com_csp_widget_EditBoxNumeric(msg + n0, 2, scade_ctx->EmitToCDS,
        scade_ctx->totalOutput);
    }
    if (scade_ctx->EmitToCDS){
      WIDGET("LeitosDisponiveis#3");
      n2 = a661_com_csp_widget_EditBoxNumeric(msg + n0 + n1, 3,
        scade_ctx->EmitToCDS, scade_ctx->availableOutput);
    }
    if (scade_ctx->EmitToCDS){
      WIDGET("LeitosOcupados#4");
      n3 = a661_com_csp_widget_EditBoxNumeric(msg + n0 + n1 + n2, 4,
        scade_ctx->EmitToCDS, scade_ctx->occupiedOutput);
    }
    if (scade_ctx->LayerVisible){
      n4 = a661_com_A661_CMD_UA_REQUEST_layer_1(msg + n0 + n1 + n2 + n3,
        scade_ctx, extern_ctx);
    }
    n5 = 4;
    WRITE_ULONG(msg + n0 + n1 + n2 + n3 + n4, A661_END_BLOCK);
    size = n0 + n1 + n2 + n3 + n4 + n5;
    WRITE_ULONG(msg + 4, size);
  }
  return n0 + n1 + n2 + n3 + n4 + n5 ;
}
 int ua_send(buffer_el msg[],  outC_UTICounts* scade_ctx, 
  extern_out_ctx_t * extern_ctx){
  int n1 = 0;
...
void ua_receive_clear(inC_UTICounts* scade_ctx,  extern_in_ctx_t * extern_ctx){
  
}
int ua_receive(buffer_el buffer[],  int buffer_size,  inC_UTICounts* scade_ctx, 
